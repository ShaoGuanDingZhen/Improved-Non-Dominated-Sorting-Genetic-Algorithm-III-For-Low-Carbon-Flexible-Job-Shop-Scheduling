...
m=ceil(rand*n);
x=M{s1(p1),s2(p1),m};
machineIndex1=sum(H(1,1:s1(p1)-1))+s2(p1);
if n>1
    while(s3(machineIndex1)==x)
        m=ceil(rand*n);
        x=M{s1(p1),s2(p1),m};
    end
end
mchrom(1,machineIndex1)=x;

n=NM{s1(p2),s2(p2)};
m=ceil(rand*n);
x=M{s1(p2),s2(p2),m};
machineIndex2=sum(H(1,1:s1(p2)-1))+s2(p2);
if n>1
    while(s3(machineIndex2)==x)
        m=ceil(rand*n);
        x=M{s1(p2),s2(p2),m};
    end
end
mchrom(1,machineIndex2)=x;
